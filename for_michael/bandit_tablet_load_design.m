...
%
% Jon Wilson
% 2015-06-30: modifed to tablet version of bandit


function c_out = bandit_tablet_load_design
...
% set format strings
data_format   = '%*f %*s %s %f %f %f';
header_format = '%*s %*s %s %s %s %s';

% read in experiment and header data
ct = 1;
while ~feof(fid)
C(ct,:) = textscan(fid,data_format,'Delimiter','\t','HeaderLines',1);
ct = ct+1;
end
frewind(fid);
