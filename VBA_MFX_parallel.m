...
while ~stop
  fprintf('On VB MFX iteration %d\n', it);
    ...
    parfor i=1:ns
        ...
        %avoid broadcasting o_group to workers
        %try
	...
	[ptemp, otemp] = VBA_NLStateSpaceModel(y{i},u{i},f_fname,g_fname,dim,options{i},in);
	p_sub{i} = ptemp; %don't think we need to clear anything from posterior
